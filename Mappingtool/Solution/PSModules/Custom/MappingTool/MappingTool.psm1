...
function Login-Azure()
{
    ...
    try 
    {
        ...
        $returnmsg.LogMsg = $returnmsg.LogMsg + "Get Automation connection name MappingToolSP.`n"
            $servicePrincipalConnection = Get-AutomationConnection -Name MappingToolSP 
        ...
        $returnmsg.LogMsg = $returnmsg.LogMsg + "Create PSCredential.`n"
            ...
            $pscredential = New-Object System.Management.Automation.PSCredential($servicePrincipalConnection.ApplicationID, $passwd)      
        ...
        if($SubscriptionID -ne "null")
        {
            ...
            $global:DefaultSubscriptionID = $servicePrincipalConnection.SubscriptionID

            $returnmsg.ReturnParameter1 = (Get-AzContext).Tenant.Id
        }

        $returnmsg.LogMsg = $returnmsg.LogMsg + "Define storage account context global variable `n" 
        $ctx = New-AzStorageContext -StorageAccountName $global:ConfApplStrAcc -UseConnectedAccount   
        $global:StorageContext = $ctx

        $returnmsg.LogMsg = $returnmsg.LogMsg + "Call function Get-Variable-Assets-UnEnc `n" 
        Get-Variable-Assets-dynamic
...
function Get-Variable-Assets-static
{
    #Hardcoded variables
    ...
    $global:ConfApplRG = "RG-MappingTool"
    #$global:ConfApplStrAcc = "appmappingtoolstracc"
    $global:ConfApplStrAcc = Get-AutomationVariable -Name "AppMappingToolStrAcc"
    $global:ConfConfigurationTable = "cfrbacperm"
}
function Get-Variable-Assets-dynamic
{            
    $configtable = Get-AzTableTable -TableName "cfmappingtool" `
                                    -resourceGroup $global:ConfApplRG `
                                    -storageAccountName $global:ConfApplStrAcc 

    $configtablerows = Get-AzTableRow -Table $configtable

    #Application configuration settings
        #Application On-Prem AD settings
        $global:ConfOUPathAADPerm = ($configtablerows | where {$_.Name -eq "Conf-AD-OUPath-AADPerm"}).Value
        $global:ConfOUPathRBACPerm = ($configtablerows | where {$_.Name -eq "Conf-AD-OUPath-RBACPerm"}).Value
        $global:ConfOUPathOldPerm = ($configtablerows | where {$_.Name -eq "Conf-AD-OUPatch-OldPerm"}).Value
        $global:ConfMainADDC = ($configtablerows | where {$_.Name -eq "Conf-AD-MainDC"}).Value
        $global:ConfAADPermissionWriteback = ($configtablerows | where {$_.Name -eq "Conf-AAD-PermissionWriteback"}).Value
            
        $global:ConfAutoAcc = ($configtablerows | where {$_.Name -eq "Conf-App-Automation-Account"}).Value
        $global:ConfLogAAcc = ($configtablerows | where {$_.Name -eq "Conf-App-Loganalytics-WS"}).Value
        $global:ConfRGReqTag = ($configtablerows | where {$_.Name -eq "Conf-App-RG-MainTag"}).Value        
        $global:ConfAppRGtoMon = ($configtablerows | where {$_.Name -eq "Conf-App-RG-to-Monitor"}).Value
        
    
        #Application tables        
        $global:ConfConfigurationTableBackup = ($configtablerows | where {$_.Name -eq "Conf-App-Configuration-TableBak"}).Value
        $global:ConfPermMappingTable = ($configtablerows | where {$_.Name -eq "Conf-App-Mapping-Table"}).Value
        $global:ConfPermIssueTable = ($configtablerows | where {$_.Name -eq "Conf-App-Config-Issues"}).Value
        
        #Application queues        
        $global:ConfCloudMsgQueue = ($configtablerows | where {$_.Name -eq "Conf-App-CL-Process-Msg-Queue"}).Value
        $global:ConfOnPremMsgQueue = ($configtablerows | where {$_.Name -eq "Conf-App-OP-Process-Msg-Queue"}).Value
        $global:ConfMembershipMsgQueue = ($configtablerows | where {$_.Name -eq "Conf-App-CL-MonMembership-Msg-Queue"}).Value
        $global:ConfOnPremMonitorConfig = ($configtablerows | where {$_.Name -eq "Conf-App-OP-MonConfig-Msg-Queue"}).Value
        $global:ConfCLMonitorConfig = ($configtablerows | where {$_.Name -eq "Conf-App-CL-MonConfig-Msg-Queue"}).Value

    #Application naming standard settings
    $global:NSAADRBACPerm = ($configtablerows | where {$_.Name -eq "NS-AAD-RBAC-Perm"}).Value
    $global:OnPremRBACRolePerm = ($configtablerows | where {$_.Name -eq "NS-AD-OnPrem-RBAC-Perm"}).Value
    $global:NSAADPerm = ($configtablerows | where {$_.Name -eq "NS-AAD-Perm"}).Value
    $global:OnPremAADRolePerm = ($configtablerows | where {$_.Name -eq "NS-AD-OnPrem-Perm"}).Value
    $global:NSAADRoleWithRolePerm = ($configtablerows | where {$_.Name -eq "NS-AAD-WithRole-Perm"}).Value
    $global:OnPremAADRoleWithRolePerm = ($configtablerows | where {$_.Name -eq "NS-AD-OnPrem-WithRole-Perm"}).Value
    $global:ConfAADOldPermPrefix = ($configtablerows | where {$_.Name -eq "Conf-AAD-OldPerm-Prefix"}).Value
    $global:ConfADOldPermPrefix = ($configtablerows | where {$_.Name -eq "Conf-AD-OldPerm-Prefix"}).Value    
}
...
function Get-LogAnalyticsMessage
{
    param (
        [parameter (Mandatory=$true)]
        [string] $LinkToSearchResults
    )
    ...
    try {        
        ...
        $returnmsg.LogMsg = $returnmsg.LogMsg + "Get Automation connection name $($global:HCAzConName).`n"
            ...
            $resourceAppIdURI = "https://api.loganalytics.io"     
            ...
            $returnmsg.LogMsg = $returnmsg.LogMsg + "Invoke-RestMethod to get oauth token `n"
                $oauth = Invoke-RestMethod -Method Post -Uri $loginURL -Body $body

                $headerParams = @{'Authorization'="$($oauth.token_type) $($oauth.access_token)"}        
                $webresults = Invoke-RestMethod -UseBasicParsing -Headers $headerParams `
                                                -Uri $LinkToSearchResults -Method Get `
                                                -Body $body -ContentType "application/json"

            $returnmsg.ReturnJsonParameters02 = $webresults.tables[0].rows | ConvertTo-Json
            $returnmsg.ReturnCode = [ReturnCode]::Success.Value__
            $returnmsg.ReturnMsg = [ReturnCode]::Success
    }
    catch {

        $returnmsg.LogMsg = $returnmsg.LogMsg + "Error in function Get-LogAnalyticsMessage. Error message: $($_.Exception.Message) `n"
        $returnmsg.ReturnCode = [ReturnCode]::Error.Value__
        $returnmsg.ReturnMsg = [ReturnCode]::Error  
    }    
...
<#Not ready
function Create-AADRoleAssignableGroup()
{
    param (
        [parameter (Mandatory=$true)]
        [string] $AADGroupName,
        [parameter (Mandatory=$true)]
        [string] $AADGroupNameMailNickName
    )
    ...
    try {
        ...
        $returnmsg.LogMsg = $returnmsg.LogMsg + "Define AppIdURI, authority and redirecturi `n"
            ...
            if($oauth.access_token)
            {
                ...
                $body = @{
                        'description'="Group created by AppMappingTool";
                        'displayName'=$AADGroupName;
                        "mailEnabled"= "false";
                        "securityEnabled"= "true";
                        "mailNickname"= $AADGroupNameMailNickName;
                ...
                $returnmsg.LogMsg = $returnmsg.LogMsg + "Invoke WebRequest to create Azure AD group with role assignment `n"

                $myresult = (Invoke-WebRequest -Method Post `
                                               -Headers @{Authorization = $token.Authorization} `
                                               -Uri $url `
                                               -UseBasicParsing `
                                               -Body $body `
            ...
            {                
                $returnmsg.LogMsg = $returnmsg.LogMsg + "Error in function Create-AADRoleAssignableGroup. Error message: Authorization Access Token is null `n"
                $returnmsg.ReturnCode = [ReturnCode]::Error.Value__
                $returnmsg.ReturnMsg = [ReturnCode]::Error 
            }
    }
        ...
        if($_.Exception.Response.StatusCode -eq "NotFound")
        {
            $returnmsg.LogMsg = $returnmsg.LogMsg + "Warning in function Create-AADRoleAssignableGroup. Warning message: $($_.Exception.Message) `n"
            $returnmsg.ReturnCode = [ReturnCode]::Warning.Value__
            $returnmsg.ReturnMsg = [ReturnCode]::Warning  
        }
        else {
            $returnmsg.LogMsg = $returnmsg.LogMsg + "Error in function Create-AADRoleAssignableGroup. Error message: $($_.Exception.Message) `n"
            $returnmsg.ReturnCode = [ReturnCode]::Error.Value__
            $returnmsg.ReturnMsg = [ReturnCode]::Error     
        }        
    }    
    ...
    try
    {
        ...
        if($null -eq (Get-AzADGroup -DisplayName $AADGroup))
        {
            $returnmsg.LogMsg = $returnmsg.LogMsg + "Create AAD Group $AADGroup `n"
                if($AADGroupType -eq "AAD")
                {
                    $newgrp = New-AzADGroup -DisplayName $AADGroup.ToLower() -MailNickname $AADGroup.ToLower()
                }
                elseif($AADGroupType -eq "AADWRole") {
                    #Offener Task!!!!!
                    #Leider noch nicht möglich, da die App Permission noch nicht existiert!
                    #Erstellen einer Role Enabled Gruppe
                    #$createadrolegroupresult = Create-AADRoleAssignableGroup -AADGroupName $AADGroup.ToLower() `
                    #                                                         -AADGroupNameMailNickName $AADGroup.ToLower()

                    #$returnmsg.LogMsg = $returnmsg.LogMsg + $createadrolegroupresult.LogMsg
                    $newgrp = New-AzADGroup -DisplayName $AADGroup.ToLower() -MailNickname $AADGroup.ToLower()
                }
...
{   
    param (
        ...
        [parameter (Mandatory=$true)]
        [ValidateSet("RG","AAD", "AADWRole","RG-Rem","AAD-Rem", "AD-Rem")]
        [string] $RequestType,
        [parameter (Mandatory=$true)]
        [string] $ADGroupName,
        [parameter (Mandatory=$false)]
...
function Get-RBAC-Mapping()
{
    ...
    try 
    {        
        if(($RequestType -eq "RG") -or ($RequestType -eq "AAD"))
        {
        ...
        elseif($RequestType -eq "RMAD")
        {
            $returnmsg.LogMsg = "Get Azure Table $MappingTableName in ResourceGroup $($global:ConfApplRG) bind to the Storage Account $($global:ConfApplStrAcc) `n"        
                $ConfPermMappingTable = Get-AzTableTable -TableName $MappingTableName -resourceGroup $($global:ConfApplRG) `
                                                         -storageAccountName $($global:ConfApplStrAcc)

            $returnmsg.LogMsg = $returnmsg.LogMsg + "Get Azure Table Row filtered by the ColumnName RBACPerm with the Value $($RBACPerm)  and PartitionKey RBAC `n"
                $mappingresult = Get-AzTableRow -Table $ConfPermMappingTable -ColumnName RBACPerm `
                                                -Value $RBACPerm -Operator Equal 
