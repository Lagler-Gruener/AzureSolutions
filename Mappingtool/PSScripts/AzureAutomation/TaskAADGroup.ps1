    ...
    {     
        ...
        if($Source -eq "RG")
        {
            Write-Output "Connect to Azure"
               $loginazureresult = Login-Azure -SubscriptionID $GroupParameters.SubscriptionID
        }
        elseif($Source -eq "AD")
        {
            Write-Output "Connect to Azure"
                ...
                if($GroupParameters.actiontype -eq "create")
                {
                    ...
                    if($createaadgroupresult.ReturnMsg -eq [ReturnCode]::Success)
                    {
                        ...
                        if($addinfotableresult.ReturnMsg -eq [ReturnCode]::Success)
                        {
                            ...
                            $rbacassigmentresult = New-AzRoleAssignment -ResourceGroupName $AZRGName -ObjectId $createaadgroupresult.ReturnParameter1 -RoleDefinitionName $GroupParameters.MappingRBACPerm -ErrorVariable rbacassignerror
